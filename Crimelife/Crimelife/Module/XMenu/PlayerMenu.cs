    ...
    class PlayerMenu : Script
    {
        ...
        public void GivePlayerMoney(Player c, string name, int money)
        {
            try
            {
                ...
                if (money < 1)
                {
                    dbPlayer.SendNotification("Warte einen Moment");
                    return;
                }
        ...
        public static void REQUEST_PEDS_PLAYER_TIE(Player c, Player target = null)
        {
            try
            {
                ...
                DbPlayer dbPlayer = c.GetPlayer();
                if (dbPlayer == null || !dbPlayer.IsValid(true) || dbPlayer.player == null || dbPlayer.IsCuffed)
                    return;
                ...
                if (dbPlayer2.GetData<int>("PBZone") == 8888) return;
                ...
                if (!dbPlayer2.IsCuffed)
                {
                    if (dbPlayer.GetItemAmount("Fesseln") >= 0)
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            ...
                            dbPlayer2.player.TriggerEvent("updateTied", new object[1] { true });
                            ...
                            NAPI.Player.SetPlayerCurrentWeapon(dbPlayer2.player, WeaponHash.Unarmed);
                        }, 4300);
                ...
                else
                {
                    ...
                    dbPlayer.player.TriggerEvent("updateTied", new object[1] { true });
                    dbPlayer2.player.TriggerEvent("updateTied", new object[1] { true });
                    ...
                    NAPI.Task.Run(() =>
                    {
                        ...
                        dbPlayer2.RefreshData(dbPlayer2);
                        dbPlayer.player.TriggerEvent("updateTied", new object[1] { false });
                        dbPlayer2.player.TriggerEvent("updateTied", new object[1] { false });
        ...
        public static void REQUEST_PEDS_PLAYER_TAKEPERSON(Player c, Player target = null)
        {
            try
            {
                ...
                if (!dbPlayer2.HasData("follow"))
                {
                    ...
                    dbPlayer2.player.TriggerEvent("toggleShooting", new object[1] { true });
                    dbPlayer2.RefreshData(dbPlayer2);
                    target.TriggerEvent("freezePlayer", false);
        ...
        public void REQUEST_PEDS_PLAYER_STABALIZE(Player c, Player target = null)
        {
            try
            {
                ...
                DbPlayer dbPlayer = c.GetPlayer();
                if (dbPlayer == null || !dbPlayer.IsValid(ignorelogin: true) || (Entity)(object)dbPlayer.player == (Entity)null || dbPlayer.IsCuffed || !dbPlayer.CanInteractAntiFlood(1) || dbPlayer.DeathData.IsDead || !dbPlayer.Medic)
                {
                    return;
                }
                if (dbPlayer.IsFarming)
                ...
                NAPI.Task.Run((Action)delegate
                {
                    if (NAPI.Pools.GetAllPlayers().Contains(dbPlayer.player))
                    {
                        ...
                        if (dbPlayer2 != null && dbPlayer2.IsValid(ignorelogin: true) && PlayerHandler.GetPlayers().Contains(dbPlayer2))
                        {
                            ...
                            WebhookSender.SendMessage("Spieler wird revived ü X menu", "Der Spieler " + dbPlayer.Name + " hat den Spieler " + dbPlayer2.Name + " revived (Fraktions MEdic).", Webhooks.revivelogs, "Revive");
                        }
                    }
                }, 10000L);
            }
