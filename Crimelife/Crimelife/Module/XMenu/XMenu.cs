...
namespace Crimelife
{
    class XMenu : Script
    {
        ...
        public void REQUEST_PEDS_PLAYER_GIVEKEY(Player client, Player destinationPlayer)
        {
            ...
            if (player != null && player2 != null && player2.IsValid() && !(((Entity)player2.player).Position.DistanceTo(((Entity)player.player).Position) > 2.5f))
            {
        ...
        public void REQUEST_VEHICLE_TOGGLE_ENGINE(Player c)
        {
            try
            {
                    ...
                    (dbVehicle.Fraktion != null && dbPlayer.Faction.Id == dbVehicle.Fraktion.Id))
                {
                    ...
                    dbVehicle.RefreshData(dbVehicle);
                    Console.WriteLine("7");
                    dbPlayer.SendNotification("Motor" + (dbVehicle.Vehicle.GetSharedData<bool>("engineStatus") ? " angeschaltet!" : " ausgeschaltet!"), 3000, dbVehicle.Vehicle.GetSharedData<bool>("engineStatus") == true ? "green" : "red");
                    Console.WriteLine("8");
                }
            }
            catch (Exception ex)
