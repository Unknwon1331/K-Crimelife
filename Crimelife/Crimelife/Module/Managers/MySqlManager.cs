...
namespace Crimelife
{
    public static class MySqlManager
    {
        ...
        public static List<ItemModel> GetStaatsbankItems(int schrank)
        {
            ...
            return list.OrderBy(x => x.Slot).ToList();
        }
        ...
        public static List<GarageVehicle> GetParkedFrakVehicles(DbPlayer dbPlayer)
        {
            List<GarageVehicle> list = new List<GarageVehicle>();
            try
            {
                MySqlQuery query = new MySqlQuery("SELECT * FROM fraktion_vehicles WHERE Parked = 1 AND Impound = 0");
                MySqlResult query2 = MySqlHandler.GetQuery(query);
                try
                {
                    MySqlDataReader reader = query2.Reader;
                    try
                    {
                        if ((reader).HasRows)
                        {
                            while ((reader).Read())
                            {
                                if (dbPlayer.Faction.Id == reader.GetInt32("FactionId"))
                                {
                                    list.Add(new GarageVehicle
                                    {
                                        Id = reader.GetInt32("Id"),
        ...
        public static void AddItem(this DbPlayer dbPlayer, uint itemId, string name, bool remove, int amount = 1, Dictionary<string, dynamic> data = null, int slot = -1, bool disablesave = false)
        {
            //IL_0038: Unknown result type (might be due to invalid IL or missing references)
            //IL_003e: Expected O, but got Unknown
            try
                    ...
                    try
                    {
                        ...
                        if (itemModel == null)
                        {
                            ...
                            itemModel = new ItemModel
                            {
                                Id = item4.Id,
                                Weight = 0,
                                Slot = dbPlayer.GetFreeInventorySlot(),
                                Name = item4.Name,
                                Amount = amount,
                                ImagePath = item4.ImagePath
                            };
        ...
        public static int GetFreeInventorySlot(this DbPlayer dbPlayer)
        {
            try
            {
                MySqlConnection val = new MySqlConnection(Configuration.connectionString);
                ...
                foreach (var itemModel in inventoryItems)
                {
        ...
        public static void AddInventoryÍtemToSlot(this DbPlayer dbPlayer, string name, int amount, int newslot = -1)
        {
            try
            {
                Item item4 = ItemModule.itemRegisterList.FirstOrDefault((Item item3) => item3.Name == name);
        ...
        public static void REmoveInventoryItemFromSlot(this DbPlayer dbPlayer, int slot, int amount)
        {
            ...
            try
            {
                ...
                try
                {
                    try
                    {
                        ((DbConnection)(object)val).Open();
                        List<ItemModel> inventoryItems = dbPlayer.GetInventoryItems();
                        ItemModel itemModel = inventoryItems.FirstOrDefault(x => x.Slot == slot);
                        ...
                        ItemModel item = new ItemModel
                        {
                            ...
                            Slot = slot,
                            Amount = itemModel.Amount - amount
                        };
