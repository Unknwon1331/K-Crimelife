...
namespace Crimelife
{
    class TeamApp : Crimelife.Module.Module<TeamApp>
    {
        ...
        public void parkCars(Player c)
        {
            ...
            DbPlayer dbPlayer = c.GetPlayer();
            if (dbPlayer == null || !dbPlayer.IsValid(ignorelogin: true) || (Entity)(object)dbPlayer.player == null || !dbPlayer.CanInteractAntiFlood(1))
            {
                return;
            }
            try
            {
                if (dbPlayer.Faction != null && dbPlayer.Faction.Id != 0 && dbPlayer.Factionrank >= 10)
                {
                    dbPlayer.Faction.GetFactionPlayers().ForEach(delegate (DbPlayer target)
                    {
                ...
                if (dbPlayer.Faction.Id != 0 && dbPlayer.Factionrank >= 11)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            dbPlayer.Faction.GetFactionPlayers().ForEach((DbPlayer dbPlayer) => dbPlayer.SendNotification(player2.Name + " ist nun Frak-Medic deiner Fraktion!", 5000, fraktion.GetRGBStr(), dbPlayer.Faction.Name));
                        }
                    }
                }
            }
            ...
            try
            {
                if (result.Reader.HasRows)
                {
                    ...
                    if (dbPlayer.Factionrank > 9)
                    {
                        ...
                        if (factionPlayers.FindAll((DbPlayer player) => player.SpielerFraktion).Count >= 35)
                        {
                            dbPlayer.SendNotification("Deine Fraktion hat mehr als 35 Member, daher kannst du keinen Mehr einladen!", 5000, dbPlayer.Faction.GetRGBStr(), dbPlayer.Faction.Name);
                        }
