            ...
            try
            {
                ...
                try {
                    while (reader.Read())
                    {
                        ...
                        if (target != null && target.IsValid(true) && target.player.Exists && !SettingsApp.isFlugmodus(target.player))
                        {
                            target.SendNotification("Neue SMS von: " + getContactName(target.player, dbPlayer.Id));
                            ...
                            clientKonversationMessage2.MessageSenderName = getContactName(target.player, dbPlayer.Id);
                            target.player.TriggerEvent("componentServerEvent", "MessengerOverviewApp", "updateChat", NAPI.Util.ToJson(clientKonversationMessage2));
                            target.player.TriggerEvent("playSMSRingtone");
                        }
                        else
                        {
