    ...
    class TelefonApp : Crimelife.Module.Module<TelefonApp>
    {
        ...
        public void CallUser(Player c, int number)
        {
            try
            {
                ...
                if (SettingsApp.isFlugmodus(dbPlayer.player))
                {
                ...
                if (SettingsApp.isFlugmodus(dbTarget.player))
                {
                    dbPlayer.SendNotification("Die angegebene Rufnummer ist derzeit nicht verfuegbar.", 3000, "red");
                    return;
                }

                if (SettingsApp.blockCalls(dbTarget.player))
                {
                ...
                c.TriggerEvent("setPhoneCallData",
                    "{\"name\":\"" + MessengerApp.getContactName(dbPlayer.player, dbTarget.Id) + "\",\"method\":\"outcomming\"}");
                dbTarget.TriggerEvent("setPhoneCallData",
                    "{\"method\":\"incoming\",\"name\":\"" + MessengerApp.getContactName(dbTarget.player, dbPlayer.Id) +
                    "\"}");
        ...
        public void CallDeclined(Player c)
        {
            try
            {
                ...
                if (call == null) return;
                call.Player1.player.TriggerEvent("cancelCall");
                call.Player2.player.TriggerEvent("cancelCall");
        ...
        public void CallAccepted(Player c)
        {
            try
            {
                ...
                call.Player1.player.TriggerEvent("componentServerEvent", "CallManageApp", "acceptCall");
                call.Player2.player.TriggerEvent("componentServerEvent", "CallManageApp", "acceptCall");
