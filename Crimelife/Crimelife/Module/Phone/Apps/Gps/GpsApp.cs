                ...
                foreach (Vehicle vehicle in NAPI.Pools.GetAllVehicles())
            {
                    ...
                    continue;
                {
                    if (NAPI.Pools.GetAllVehicles().FirstOrDefault(x => x.NumberPlate == dbVehicle.Plate) != null)
                        if ((dbVehicle.OwnerId == dbPlayer.Id || dbVehicle.Keys.Contains(dbPlayer.Id)) && vehicle.Position.DistanceTo(dbPlayer.player.Position) < 12000)
                        {
                        ...
                        else if (dbPlayer.VehicleKeys.ContainsKey(dbVehicle.Id) && vehicle.Position.DistanceTo(dbPlayer.player.Position) < 12000)
                        {
