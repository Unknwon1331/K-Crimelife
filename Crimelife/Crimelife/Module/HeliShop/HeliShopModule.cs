        ...
        public static void Helihandel(Player client, string selection)
        {
            ...
            try
            {
                ...
                if (dbPlayer.Money >= buyCar.Price)
                {
                    ...
                    WebhookSender.SendMessage("Spieler kauft Fahrzeug", "Der Spieler " + dbPlayer.Name + " kauft das Fahrzeug " + dbVehicle.Model + " für " + buyCar.Price + "$.", Webhooks.buylogs, "Fahrzeugshop");
                }
                else
                {
                    client.TriggerEvent("componentServerEvent", "NativeMenu", "hide");
