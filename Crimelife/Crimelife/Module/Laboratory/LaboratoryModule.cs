    ...
    class LaboratoryModule : Crimelife.Module.Module<LaboratoryModule>
    {
                ...
                try
                {
                    if ((reader).HasRows)
                    {
                        while ((reader).Read())
                        {
                            ...
                            NAPI.Blip.CreateBlip(499, laboratory.Entrance, 0.8f, (byte)FactionModule.getFactionById(laboratory.FactionId).Blip, "Labor - " + FactionModule.getFactionById(laboratory.FactionId).Name, 255, 0, true, 0);
        ...
        public void enterLaboratory(Player c, int Id)
        {
            try
            {
                ...
                if (player == null || !player.IsValid(ignorelogin: true) || (Entity)(object)player.player == (Entity)null || player.Faction.Id == 0)
                {
                    return;
                }
                ...
                {
                    bool flag = player.GetData<bool>("IN_LABOR");
                    if (!flag)
                    {
                        ...
                        int num2 = 1;
                        int num3 = 1;
                        player.player.TriggerEvent("loadMethInterior", new object[3] { num2, num, num3 });
                    }
                }
            }
            catch (Exception ex)
        ...
        public void leaveLaboratory(Player c)
        {
            try
            {
                ...
                if (dbPlayer == null || !dbPlayer.IsValid(ignorelogin: true) || (Entity)(object)dbPlayer.player == (Entity)null || dbPlayer.Faction.Id == 0)
                {
                    return;
                }
                ...
                {
                    bool flag = dbPlayer.GetData<bool>("IN_LABOR");
                    if (flag)
                    {
        ...
        public void processBattery(Player c)
        {
            try
            {
                ...
                Laboratory laboratory = laboratories.FirstOrDefault((Laboratory labor) => labor.FactionId == dbPlayer.Faction.Id);
                if (laboratory == null || dbPlayer.Faction.Id != laboratory.FactionId || !dbPlayer.HasData("IN_LABOR") || !(bool)dbPlayer.GetData<bool>("IN_LABOR") || dbPlayer.IsFarming)
                {
                    return;
                }
        ...
        public void openMaterialStorage(Player c)
        {
            try
            {
                ...
                if (laboratory != null && dbPlayer.Faction.Id == laboratory.FactionId && dbPlayer.HasData("IN_LABOR"))
                {
                    ...
                    if ((bool)dbPlayer.GetData<bool>("IN_LABOR"))
                    {
        ...
        public void openLabor(Player c)
        {
            try
            {
                ...
                {
                    Laboratory laboratory = laboratories.FirstOrDefault((Laboratory labor) => labor.FactionId == dbPlayer.Faction.Id);
                    if (laboratory != null && dbPlayer.Faction.Id == laboratory.FactionId && dbPlayer.HasData("IN_LABOR") && (bool)dbPlayer.GetData<bool>("IN_LABOR"))
                    {
