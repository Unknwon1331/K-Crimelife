    ...
    class GarageModule : Crimelife.Module.Module<GarageModule>
    {
        ...
        public static void loadGarage(MySqlDataReader reader)
        {
            try
            {
                ...
                ColShape val = NAPI.ColShape.CreateCylinderColShape(garage.Position, 3.4f, 1.4f, 0);
                val.SetData("FUNCTION_MODEL", new FunctionModel("openGarage", reader.GetInt32("id"), garage.Name));
                val.SetData("MESSAGE", new Message("Benutze E um die Garage zu öffnen.", garage.Name, "#08c7cf", 5000));

                NAPI.Ped.CreatePed(NAPI.Util.GetHashKey("csb_reporter"), garage.Position, garage.Rotation2, true, true, true, true, 0);
                NAPI.Blip.CreateBlip(473, garage.Position, 1f, 0, garage.Name, 255, 0.0f, true, 0, 0);
            }
            catch (Exception ex)
            {
                Logger.Print("[EXCEPTION loadGarage] " + ex.Message);
                Logger.Print("[EXCEPTION loadGarage] " + ex.StackTrace);
            }
        }
        ...
        public void openFraktionsGarage(Player c, int id, string name)
        {
            try
            {
                ...
                try
                {
                    if (name == null) return;
                    if (id == 0) return;
                    if (dbPlayer.Faction.Short == null) return;

                    if (c.Dimension == 9999 && id == 5564)
                    {
                        List<NativeItem> nativeItems = new List<NativeItem>
                        {
                            new NativeItem("[GW] Vehicles", "vehiclemenu"),
                            new NativeItem("[GW] Waffenauswahl", "weapons")
                        };
                        NativeMenu nativeMenu = new NativeMenu("Gangwar Selection", "", nativeItems);
                        dbPlayer.ShowNativeMenu(nativeMenu);
                        return;
                    }
                    if (dbPlayer.Faction.Id != id) return;
                    c.TriggerEvent("openWindow", "Garage", "{\"id\":" + id + ", \"name\": \"" + dbPlayer.Faction.Short + "\", \"fraktion\":true}");
        ...
        [RemoteEvent("nM-Gangwar Selection")]
        public void GangwarSelection(Player c, string selection)
        {
            try
            {
                if (!(c == null))
                {
                    ...
                    if (selection == "weapons")
                    {
                        ...
                        {
                            player.TriggerEvent("client:createKitBrowser", false);
                        }
                    }
                    if (selection == "vehiclemenu")
                    {
                        player.CloseNativeMenu();
                        if (c.Dimension == 9999 && (GangwarModule.RunningGangwar != null))
                        {
                            List<NativeItem> nativeItems = new List<NativeItem>
                        {
                            new NativeItem("Revolter", "revolter"),
                            new NativeItem("BF400", "motorrad")
                        };
                            NativeMenu nativeMenu = new NativeMenu("Gangwar Vehicles", "Wähle ein Fahrzeug aus", nativeItems);
            ...
            {
                Logger.Print("[EXCEPTION nM-GangwarSelection] " + ex.Message);
                Logger.Print("[EXCEPTION nM-GangwarSelection] " + ex.StackTrace);
            }
        }

        [RemoteEvent("nM-Gangwar Vehicles")]
        public void GangwarVehicles(Player c, string selection)
        {
            try
            {
                if (!(c == null))
                {
                    ...
                    if (selection == "revolter")
                    {
                        ...
                        if (GangwarModule.RunningGangwar != null)
                        {
                            if (player.Faction == GangwarModule.RunningGangwar.Faction)
                            {
                                ...
                                Vehicle val = NAPI.Vehicle.CreateVehicle(0xE78CC3D9, GangwarModule.RunningGangwar.DefenderVehicleSpawn, GangwarModule.RunningGangwar.DefenderVehicleRotation, 0, 0, "", 255, false, true, c.Dimension);
                                ...
                                player.SendNotification("Gangwar Revolter ausgeparkt", 3000, "darkgrey", "GANGWAR");
                            }
                            else if (player.Faction == GangwarModule.RunningGangwar.Attacker)
                            {
                                var alreadyVehicle = NAPI.Pools.GetAllVehicles().ToList().FirstOrDefault(x => x != null && x.Exists && x.HasData("ownerId") && x.GetData<DbPlayer>("ownerId") == player);
                                if (alreadyVehicle != null) alreadyVehicle.Delete();
                                Vehicle val = NAPI.Vehicle.CreateVehicle(0xE78CC3D9, GangwarModule.RunningGangwar.AttackerVehicleSpawn, GangwarModule.RunningGangwar.AttackerVehicleRotation, 0, 0, "", 255, false, true, c.Dimension);
                                val.CustomPrimaryColor = player.Faction.RGB;
                                val.CustomSecondaryColor = player.Faction.RGB;
                                val.NumberPlate = player.Faction.Short.ToUpper();
                                val.SetData("ownerId", player);
                    ...
                    if (selection == "motorrad")
                    {
                        ...
                        if (GangwarModule.RunningGangwar != null)
                        {
                            if (player.Faction == GangwarModule.RunningGangwar.Faction)
                            {
                                var alreadyVehicle = NAPI.Pools.GetAllVehicles().ToList().FirstOrDefault(x => x != null && x.Exists && x.HasData("ownerId") && x.GetData<DbPlayer>("ownerId") == player);
                                if (alreadyVehicle != null) alreadyVehicle.Delete();
                                Vehicle val = NAPI.Vehicle.CreateVehicle(0x5283265, GangwarModule.RunningGangwar.DefenderVehicleSpawn, GangwarModule.RunningGangwar.DefenderVehicleRotation, 0, 0, "", 255, false, true, c.Dimension);
                                val.CustomPrimaryColor = player.Faction.RGB;
                                val.CustomSecondaryColor = player.Faction.RGB;
                                val.NumberPlate = player.Faction.Short.ToUpper();
                                val.SetData("ownerId", player);
                                c.SetIntoVehicle(val, -1);
                                player.SendNotification("Gangwar BF400 ausgeparkt", 3000, "darkgrey", "GANGWAR");
                            }
                            else if (player.Faction == GangwarModule.RunningGangwar.Attacker)
                            {
                                var alreadyVehicle = NAPI.Pools.GetAllVehicles().ToList().FirstOrDefault(x => x != null && x.Exists && x.HasData("ownerId") && x.GetData<DbPlayer>("ownerId") == player);
                                if (alreadyVehicle != null) alreadyVehicle.Delete();
                                Vehicle val = NAPI.Vehicle.CreateVehicle(0x5283265, GangwarModule.RunningGangwar.AttackerVehicleSpawn, GangwarModule.RunningGangwar.AttackerVehicleRotation, 0, 0, "", 255, false, true, c.Dimension);
                                val.CustomPrimaryColor = player.Faction.RGB;
                                val.CustomSecondaryColor = player.Faction.RGB;
                                val.NumberPlate = player.Faction.Short.ToUpper();
                                val.SetData("ownerId", player);
            ...
            {
                Logger.Print("[EXCEPTION nM-GangwarSelection2] " + ex.Message);
                Logger.Print("[EXCEPTION nM-GangwarSelection2] " + ex.StackTrace);
            }
        }
                    ...
                    case 0:
                        if (fraktion)
                        {
                            foreach (Vehicle vehicle in NAPI.Pools.GetAllVehicles())
                            {
                                ...
                                if (dbVehicle.Fraktion != null && dbVehicle.Fraktion.Id == dbPlayer.Faction.Id && vehicle.Position.DistanceTo(dbPlayer.player.Position) < 30)
                                {
                        ...
                        else
                        {
                            foreach (Vehicle vehicle in NAPI.Pools.GetAllVehicles())
                            {
                                ...
                                if ((dbVehicle.OwnerId == dbPlayer.Id || dbVehicle.Keys.Contains(dbPlayer.Id) || dbPlayer.VehicleKeys.ContainsKey(dbVehicle.Id)) && vehicle.Position.DistanceTo(dbPlayer.player.Position) < 30)
                                {
                                    list.Add(new GarageVehicle
                                    {
                                        Id = dbVehicle.Id,
        ...
        public Vehicle? requestVehicle(Player c, string state, int garageid, int vehicleid, bool fraktion)
        {
            ...
            {
                DbPlayer player = c.GetPlayer();
                if ((player == null || !player.IsValid(true) ? false : player.player != null))
                {
