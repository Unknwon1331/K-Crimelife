    ...
    class VoiceModule : Crimelife.Module.Module<VoiceModule>
    {
        ...
        public static void changeVoiceRange(Player c)
        {
            try
            {
                ...
                {
                    dbPlayer.SendNotification("Du hast deine Sprachreichweite verändert 3 Meter", 3000, "#1dd604", "");                                      
                }
                else
                if (range == 2)

                {
                    dbPlayer.SendNotification("Du hast deine Sprachreichweite verändert 16 Meter", 3000, "#de8304", "");
                }
                else
                if (range == 3)

                {
                    dbPlayer.SendNotification("Du hast deine Sprachreichweite verändert 32 Meter", 3000, "#b50707", "");
                }

                dbPlayer.SetSharedData("CLIENT_RANGE", range);
                //dbPlayer.SendNotification("Du hast deine Sprachreichweite verändert " + range + "", 3000);
                dbPlayer.player.TriggerEvent("setVoiceType", range.ToString());
            }
            catch (Exception ex)
            {
