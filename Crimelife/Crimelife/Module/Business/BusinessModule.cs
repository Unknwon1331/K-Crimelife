    ...
    internal class BusinessModule : Module<BusinessModule>
    {
        ...
        protected override bool OnLoad()
        {
            ...
            try
            {
                ...
                NAPI.Blip.CreateBlip(475, EntrancePosition, 1f, (byte)0, "Businesstower", byte.MaxValue, 0f, true, (short)0, 0u);
                return true;
            }
            finally
            {
        ...
        public void Businesstower(Player c, string selection)
        {
            try
            {
                ...
                DbPlayer player = c.GetPlayer();
                if (player == null || !player.IsValid(ignorelogin: true) || (Entity)(object)player.player == (Entity)null || string.IsNullOrEmpty(selection))
                {
                    return;
                }
                ...
                switch (selection)
                {
                    ...
                    case "buy":
                        if (player.Business.Id != 0)
                        {
                            player.SendNotification("Du bist bereits in einem Business!", 3000, "orange", "BUSINESS");
                            break;
                        }
                        player.OpenTextInputBox(new TextInputBoxObject
                        {
                            Title = "Business beantragen [10.000.000$]",
                            Message = "Gib den Namen für dein Business ein, damit du den Antrag für dein Business stellen kannst.",
        ...
        public void createBusiness(Player c, string text)
        {
            try
            {
                ...
                DbPlayer player = c.GetPlayer();
                if (player == null || !player.IsValid(ignorelogin: true) || (Entity)(object)player.player == (Entity)null || string.IsNullOrEmpty(text))
                {
                    return;
                }
                if (player.Money >= 10000000)
                {
                    ...
                    if (flag)
                    {
                        ...
                        WebhookSender.SendMessage("Business Create", "Spieler: " + player.Name + " hat das Business: " + business3.Name + " Erstellt", Webhooks.buylogs, "BUSINESS");
                    }
                    else
                    {
                        player.player.SendNotification("Entweder dieses Business ist bereits vergeben, der Name ist länger als 32 Zeichen oder es sind Sonderzeichen enthalten.", true);
                    }
