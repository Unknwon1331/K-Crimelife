	...
	internal class BusinessApp : Script
	{
		...
		public static void requestBusinessMembers(Player c)
		{
			try
			{
				...
				if (player == null || !player.IsValid(ignorelogin: true) || (Entity)(object)player.player == (Entity)null || !player.CanInteractAntiFlood(1) || player.Business.Id == 0)
				{
					return;
				}
		...
		public static void requestBusinessMOTD(Player c)
		{
			try
			{
				...
				{
					DbPlayer player = c.GetPlayer();
					if (player != null && player.IsValid(ignorelogin: true) && !((Entity)(object)player.player == (Entity)null) && player.Business.Id != 0)
					{
		...
		public void saveBusinessMOTD(Player c, string motd)
		{
			try
			{
				...
				DbPlayer player = c.GetPlayer();
				if (player != null && player.IsValid(ignorelogin: true) && !((Entity)(object)player.player == (Entity)null) && player.CanInteractAntiFlood(1) && player.Business.Id != 0 && player.Businessrank >= 1)
				{
					if (motd.Length > 32)
					{
