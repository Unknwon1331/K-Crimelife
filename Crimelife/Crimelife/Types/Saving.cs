    ...
    class Saving
    {
                ...
                foreach (DbPlayer dbPlayer in PlayerHandler.GetPlayers())
                {
                    ...
                    if (dbPlayer.DeathData.IsDead)
                    {
                        ...
                        if (DateTime.Now.Subtract(dateTime).TotalMinutes >= 2)
                        {
                            ...
                            dbPlayer.disableAllControls(false);
                            dbPlayer.disableAllPlayerActions(false);
                            dbPlayer.player.TriggerEvent("toggleShooting", new object[1] { false });

                            if (dbPlayer.player.Dimension != FactionModule.GangwarDimension)
                            {
        ...
        public static void UpdateDbPositions()
        {
            ...
            try
            {
                foreach (DbPlayer dbPlayer in PlayerHandler.GetPlayers())
                {
                    if (dbPlayer.Dimension < 3500 && dbPlayer.Position.DistanceTo(new Vector3(402.8664, -996.4108, -99.00027)) > 5.0f) //FFA Abfrage
                    {
                        MySqlQuery mySqlQuery = new MySqlQuery($"UPDATE accounts SET Location = '{NAPI.Util.ToJson(dbPlayer.player.Position)}' WHERE Id = @id");
