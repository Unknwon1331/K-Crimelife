...
namespace Crimelife
{
    public class SyncThread
    {
        ...
        public void Start()
        {
            ...
            //////////////////////////////////////////
            ///
            Timer FünfzenMinTimer = new Timer
            {
                Interval = 900000,
                AutoReset = true,
                Enabled = true
            };
            FünfzenMinTimer.Elapsed += delegate (object sender, ElapsedEventArgs args)
            {
                try
                {
                    //NAPI.World.SetWeather(Weather.EXTRASUNNY);
                ...
                {
                    Logger.Print("[EXCEPTION CheckFiveZehnMin]" + ex.Message);
                    Logger.Print("[EXCEPTION CheckFiveZehnMin]" + ex.StackTrace);
                }
            };
            //////////////////////////////////////////////
        ...
        public static void CheckFiveSec()
        {
            try
            {
                ...
                /*foreach (Player player in NAPI.Pools.GetAllPlayers())
                {
                    ...
                    {
                        if (player.HasData("spectatedPlayer"))
                        {
                            string name = dbplayer.GetStringData("spectatedPlayer");
                            Player client = NAPI.Player.GetPlayerFromName(name);
                            dbplayer.Position = client.Position;
                        }
                    }
                }*/
